...
//! do not know, then MOSS with the small tempo model beside it, then
//! the assistant for all the writing.
//! A single song is the same job with one item.
//!
//! Every song keeps where it stands - lyrics wanted, found or done; style
//! wanted, heard or done - and the job itself is kept on disk, so a restart
...
async fn run(state: &AppState, job: &Job) -> anyhow::Result<()> {
    ...
    let outcome = async {
        ...
        if job.style && !stopped() {
            write_styles(state, job, can_listen).await?;
        }
        anyhow::Ok(())
    }
    .await;
        ...
        if !config.available() || matches!(config.provider, lyrics_sync::AsrProvider::None) {
            ...
            match crate::vocal_separator(state).await {
                Some(separator) if !to_separate.is_empty() => {
                    ...
                    if matches!(state.separation_config.read().await.runtime, lyrics_sync::OnnxFlavour::Cpu) && crate::hardware::hardware().cuda.is_some() {
...
/// The listening: MOSS over every song in one load, the tempo measured
/// beside it. What it heard is stored with each song the moment it is heard.
async fn listen_branch(state: &AppState, job: &Job, style_items: &[&training::DatasetItem], can_listen: bool) -> anyhow::Result<()> {
    ...
    if !style_items.is_empty() {
        ...
        // the tempo is measured ahead of MOSS: four threads decode, Beat This!
        // - loaded once, on the card - measures. Each song's
        ...
        let log = tokio::task::spawn_blocking(move || {
            listen::hear_batch(&captioner, &moss, Some(&libraries), &audio, &task_cancel, |index, caption| {
                ...
                let outcome = measured.map_err(|error| anyhow::anyhow!("measure the tempo: {error}")).and_then(|facts| {
                    let heard = listen::heard(&caption?, &facts)?;
                    Ok((heard, facts))
                });
                match outcome {
                    Ok((heard, facts)) => {
                        // the heard description, numbers measured, until the assistant writes the final line
                        let draft = if heard.genre.is_empty() { heard.caption.clone() } else { format!("{}. {}", heard.genre, heard.caption) };
                        let note = training::HeardNote { genre: heard.genre, caption: heard.caption, bpm: facts.bpm };
                        let patch = training::ItemPatch { style: Some(draft), style_state: Some(training::StyleState::Heard), heard: Some(Some(note)), ..Default::default() };
                        if let Err(error) = training.update_item(&dataset_id, item, patch) {
        ...
        for lane in measuring {
            lane.join().map_err(|_| anyhow::anyhow!("measuring the tempo failed"))?;
        }
        if stopped() {
            return Ok(());
        }
...
/// The style sentence of every song heard, written with its final lyrics.
async fn write_styles(state: &AppState, job: &Job, can_listen: bool) -> anyhow::Result<()> {
    use training::StyleState;
    let (shared, cancel) = (state.prepare.clone(), state.prepare_cancel.clone());
    let heard: Vec<training::DatasetItem> = songs(state, job)?.into_iter().filter(|item| item.style_state == StyleState::Heard).collect();
    if heard.is_empty() {
        return Ok(());
    }
    stage(&shared, "styles", heard.iter().map(|item| item.id.clone()).collect());
    for song in &heard {
        if cancel.load(Ordering::Relaxed) {
            return Ok(());
        }
        let outcome = match &song.heard {
            Some(note) => {
                let heard = listen::Heard { genre: note.genre.clone(), caption: note.caption.clone() };
                describe(state, &heard, &audio_facts::Facts { bpm: note.bpm }, song).await
            }
            None => Err("nothing heard is kept for this song; describe it again".to_string()),
        };
        match outcome {
            Ok(style) => {
        ...
        progress(&shared, "styles", |done| done + 1);
        settle_pending(&state.training, job, &shared, can_listen, Passed::Writing);
    }
    end_stage(&shared, "styles");
    Ok(())
}
...
async fn describe(state: &AppState, heard: &listen::Heard, facts: &audio_facts::Facts, song: &training::DatasetItem) -> Result<String, String> {
    ...
    for _ in 0..2 {
        let content = crate::assistant_ask(state, listen::YUE2_SYSTEM_PROMPT, &prompt, None, assistant::AssistTarget::Style)
            .await
            .map_err(|(_, Json(error))| error.error)?;
    ...
    Ok(best.expect("at least one answer").1)
}
