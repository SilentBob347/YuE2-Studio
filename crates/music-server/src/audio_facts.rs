//! Tempo measured from the recording itself.
//!
//! A captioning model hears a song well and its numbers badly: MOSS called a
//! 90 BPM song 102 BPM. So the tempo comes from a model built for it, run
//! through ONNX on the card when it takes it. The key is not measured: YuE2's
//! style caption never states it, and the score's K: comes from SheetSage2.
//!
...
/// Beat This! loaded once for a whole batch of songs.
pub struct Measurer {
    beat: Session,
...
impl Measurer {
    ...
    pub fn measure_mono(&mut self, mono: &[f32]) -> Result<Facts> {
        ...
        Ok(Facts { bpm: tempo(mono, &mut self.beat)? })
    }
}
...
mod tests {
    ...
    fn measures_real_songs() {
        ...
        for song in std::env::var("AUDIO_FACTS_SONGS").expect("AUDIO_FACTS_SONGS").split(';') {
            ...
            println!("{song}: {} BPM ({:.1} s)", facts.bpm, started.elapsed().as_secs_f32());
        }
    }
