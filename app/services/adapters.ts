  ...
  useEffect(() => {
    const read = () =>
      void fetchAdapters()
        ...
        .catch(() => undefined);
    read();
    window.addEventListener('yue:adapters-changed', read);
    window.addEventListener('yue:models-changed', read);
    return () => {
      window.removeEventListener('yue:adapters-changed', read);
      window.removeEventListener('yue:models-changed', read);
    };
  }, []);
  return library;
}
